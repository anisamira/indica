...
$sqliu="SELECT * FROM session where session_name='$sesi'";
      ...
      fputcsv($output, array('Module Name','Session','Goal','Strategies', 'Action Plan', 'KPI','Operation Definition','Achievement 2014','Achievement 2015','Target '.$year1.'','Target '.$year2.'','Target '.$year3.'','Target '.$year4.'','Target '.$year5.'','Ownership','Data Source','Estimated Cost','Expected Financial Return'));  
      $sql = "SELECT module.module_name,goal.session_name,goal.goal_desc,strategy.strategy_desc,actionplan.actionplan_desc,kpi.kpi_desc,kpi.operation_def,baseline.baseline1,baseline.baseline2,target.target1,target.target2,target.target3,target.target4,target.target5,reference.ownership,reference.data_source,reference.estimated_cost,reference.exp_fin_return
	                                        FROM goal 
